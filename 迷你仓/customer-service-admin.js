(function (root, factory) {
  ...
  function buildRowsMarkup(type, language, records) {
    ...
    return records.map(source => {
      ...
      if (type === 'phone') {
        return `<tr class="customer-service-row" data-record-id="${escapeHtml(item.id)}"><td><b>${escapeHtml(item.id)}</b></td><td>${escapeHtml(createdAt)}</td><td><b>${escapeHtml(item.customer.name)}</b><small>${escapeHtml(item.customer.id)} · ${escapeHtml(item.customer.phone)}</small></td><td>${escapeHtml(item.direction)}</td><td>${escapeHtml(item.robot)}</td><td>${escapeHtml(item.duration)}</td><td><span class="cs-status">${escapeHtml(item.result)}</span></td><td><div class="cs-clamp">${escapeHtml(item.summary)}</div></td><td><div class="cs-clamp">${escapeHtml(item.task.title)}</div></td><td class="cs-actions-cell"><button type="button" class="cs-link" data-cs-action="detail" data-record-id="${escapeHtml(item.id)}">${escapeHtml(copy.actions.view)}</button></td></tr>`;
      }
      const tags = Object.values(item.tags || {}).flat().slice(0, 2).map(tag => `<span class="cs-tag">${escapeHtml(tag)}</span>`).join('');
      return `<tr class="customer-service-row" data-record-id="${escapeHtml(item.id)}"><td><b>${escapeHtml(item.id)}</b></td><td>${escapeHtml(createdAt)}</td><td><b>${escapeHtml(item.customer.name)}</b><small>${escapeHtml(item.customer.id)} · ${escapeHtml(item.customer.phone)}</small></td><td>${tags}</td><td>${escapeHtml(item.channel)}</td><td><div class="cs-clamp">${escapeHtml(item.summary)}</div></td><td><span class="cs-tag intent">${escapeHtml(item.intent.type)}</span></td><td><div class="cs-references">${referenceMarkup(item.references, language)}</div></td><td><div class="cs-clamp">${escapeHtml(item.unresolved || '—')}</div></td><td><div class="cs-clamp">${escapeHtml(item.task.title)}</div></td><td>${escapeHtml(item.owner)}</td><td>${escapeHtml(item.lastAt)}</td><td class="cs-actions-cell"><button type="button" class="cs-link" data-cs-action="detail" data-record-id="${escapeHtml(item.id)}">${escapeHtml(copy.actions.view)}</button></td></tr>`;
    }).join('');
  }
  ...
  function tableHeaders(type, copy) {
    ...
    return keys.map(key => `<th${key === 'action' ? ' class="cs-actions-cell"' : ''}>${escapeHtml(copy.columns[key])}</th>`).join('');
  }
